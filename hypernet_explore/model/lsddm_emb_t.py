...
class Dynamics(nn.Module):
    ...
    def forward(self, x):
        ...
        out_dim = total_dim - (self.task_emb_dim+1)
        rv = rv[:, :out_dim]
...
def configure(props):
    ...
    # fhat = nn.Sequential(nn.Linear(lsd+task_emb_dim+explicit_time, h_dim), nn.ReLU(),
    ...
    fhat = nn.Sequential()
    fhat.add_module('layer_0', nn.Linear(lsd+task_emb_dim+explicit_time, h_dim))
    ...
    fhat.add_module('layer_out', nn.Linear(h_dim, lsd+task_emb_dim))
    ...
    if "projfn" in props:
        if props["projfn"] == "PSD-REHU":
            V = MakePSD(ICNN([lsd+task_emb_dim+explicit_time, ph_dim, ph_dim, 1], activation=ReHU(float(props["rehu"]) if "rehu" in props else 0.01)), 
                             lsd+task_emb_dim+explicit_time, 
                             eps=projfn_eps, 
                             d=1.0)
        else:
            pass

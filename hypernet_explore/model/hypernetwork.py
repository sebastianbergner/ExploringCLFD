...
from hypernet_explore.model.initializers import *
...
def str_to_init(init_str):
    """Convert the name of an init function into the actual
    init function.
    Args:
        init_str: Name of init function (as defined by command-line
            arguments).
    Returns:
        init function (first arg: weights) instance.
    """
    if init_str == "kaiming":
        init = kaiming
    elif init_str == "xavier":
        init = xavier
    elif init_str == "principled_weight_init_normal":
        init = principled_weight_init_normal
    elif init_str == "principled_weight_init_uniform":
        init = principled_weight_init_uniform
    else:
        raise Exception("Init function %s unknown." % init_str)
    return init

def str_to_optim(optim_str):
    """Convert the name of an optimizer function into the actual
    optimizer function.
    Args:
        init_str: Name of optimizer function.
    Returns:
        optimizer function instance.
    """
    if optim_str == "Adam":
        ...
        raise Exception("Optimizer function %s unknown." % optim_str)
    return optim

def init_params(weights, bias, init_fn, init_bias_pwi, last_layer = None):
    # Initialize weights
    additional_params = None
    if init_fn == principled_weight_init_uniform or init_fn == principled_weight_init_normal:
        assert last_layer != None
        use_PWI_bias_init = True if init_bias_pwi else False
        additional_params = (last_layer, use_PWI_bias_init)

        if use_PWI_bias_init:
            #initialize bias to non-zero according to PWI paper
            fan_in_last_layer, _ = nn.init._calculate_fan_in_and_fan_out(last_layer)
            c_relu = 1 if weights is last_layer else 2
            var = c_relu/(2 * fan_in_last_layer)
            std = math.sqrt(var)
            a = math.sqrt(3.0) * std
            nn.init.uniform_(bias, -a, a)
        else:
            #bias = nn.init.zeros_(bias) # according to paper
    ...
    init_fn(weights, additional_params)
    ...
    for group in optimizer.param_groups:
        for p in group["params"]:
            ...
            if amsgrad and not detach_dp:
                raise ValueError(
                    "Cannot backprop through optimizer step if "
                    + '"amsgrad" is enabled.'
                )
...
def rmsprop_step(optimizer, detach_dp=True):
    ...
    for group in optimizer.param_groups:
        for p in group['params']:
            ...
            """hypernetwork.py:380: UserWarning: This overload of addcmul is deprecated:
                addcmul(Number value, Tensor tensor1, Tensor tensor2)
                Consider using one of the following signatures instead:
                addcmul(Tensor tensor1, Tensor tensor2, *, Number value)"""
        ...
        for p in group['params']:
            ...
            if momentum != 0:
                ...
                if 'momentum_buffer' not in param_state:
                    ...
                    """hypernetwork.py:480: UserWarning: This overload of add_ is deprecated:
                    add_(Number alpha, Tensor other)
                    Consider using one of the following signatures instead:
...
class HyperNetwork(nn.Module):
    def __init__(
        ...
        self.init_bias_pwi = init_bias_pwi
        ...
        if verbose:
            print(
                f"Constructed hypernetwork \n"
                + f"num_task_emb: {int(np.sum([t.numel() for t in self.get_task_embs()]))} \n"
                + f"Weights and biases: {self.theta_shapes} \n"
                + f"Total parameters: {shapes_to_num_weights(self.theta_shapes)} \n"
                + f"Total outputs: {shapes_to_num_weights(self.target_shapes)} \n"
            )

    def init_batch_entropy_new_task(self):
        # Create a new array to store batch entropies
        self.batch_entropy_list = list()
        self.compute_batch_entropy = True

    def store_batch_entropy(self, save_dir, task_id):
        # Save self.batch_entropy_list
        batch_entropy_np = np.array(self.batch_entropy_list)
        np.save(os.path.join(save_dir, f"be_taskid_{task_id}.npy"), batch_entropy_np)
        self.compute_batch_entropy = False
    ...
    def gen_new_task_emb(self):
        ...
        self.task_embs.append(
            nn.Parameter(data=torch.Tensor(self.te_dim), requires_grad=True)
        )
        torch.nn.init.normal_(self.task_embs[-1], mean=0.0, std=1.0)
    ...
    def gen_layers(self):
        ...
        for i in range(0, len(self.theta), 2):
            init_params(self.theta[i], self.theta[i + 1], self.init_fn, self.init_bias_pwi, self.theta[-2])
    ...
    def chunk_forward(self, task_id, dTheta=None, squeeze=True, ext_inputs=None):
        ...
        if self.compute_batch_entropy:
            step_batch_entropies = list()
        ...
        for i in range(0, len(self.hidden_dims), 2):
            ...
            # Compute batch entropy for hidden layers
            if self.compute_batch_entropy:
                with torch.no_grad():
                    be = batch_entropy(h)
                    step_batch_entropies.append(be)
        ...
        for i in range(len(self.hidden_dims), len(self.theta_shapes[1:]), 2):
            ...
            # Compute batch entropy for output layer
            if self.compute_batch_entropy:
                with torch.no_grad():
                    be = batch_entropy(W)
                    step_batch_entropies.append(be)
        ...
        if self.compute_batch_entropy:
            # print(f"len={len(self.batch_entropy_list)}")
            # print(step_batch_entropies)
            # Append the batch entropies for the latest forward pass
            self.batch_entropy_list.append(step_batch_entropies)
...
class ChunkedHyperNetwork(HyperNetwork):
    def __init__(
        ...
        self.chunk_embs = nn.Parameter(
            torch.Tensor(self.num_chunks, ce_dim), requires_grad=True
        )
        nn.init.normal_(self.chunk_embs, mean=0.0, std=1.0)
    ...
    def forward(self, task_id, dTheta=None, squeeze=True):
        ...
        weights = self.chunk_forward(
            task_id=task_id, dTheta=hnet_dTheta, ext_inputs=chunk_embs
        )
...
class ChunkedTaskEmbHypernetwork(HyperNetwork):
    def __init__(
        self,
        target_shapes,
        layers=...,
        ...
        # The task embeddings are chunked
        # We do not use a separate chunk embedding
        # Thats why ce_dim is passed as None to super()
        super().__init__(
            [[chunk_dim]],
            layers,
            te_dim,
        ...
        self.chunk_dim = chunk_dim
        self.final_target_shapes = target_shapes
        self.num_outputs = shapes_to_num_weights(target_shapes)
        # num_chunks is the batch dimension
        self.num_chunks = int(np.ceil(self.num_outputs / self.chunk_dim))

        hnet_dims = self.theta_shapes

        # Total size of regularized parameters
        num_theta = shapes_to_num_weights(hnet_dims)

        # Chunked task embeddings are only trained for specific tasks
        # and then frozen for other tasks (they are not regularized)
        num_task_emb = int(np.sum([t.numel() for t in self.get_task_embs()]))

        # Total parameters of the hypernetwork
        # num_theta: regularized parameters (weights,biases)
        # num_task_emb: Non-regularized chunked task embeddings
        self.num_weights = num_theta + num_task_emb
        ...
        if verbose:
            print(
                f"Constructed hypernetwork \n"
                + f"num_task_emb: {num_task_emb} \n"
                + f"chunked task embedding shape: {(self.num_chunks, self.te_dim)} \n"
                + f"Weights and biases: {shapes_to_num_weights(hnet_dims)} \n"
                + f"Total parameters: {self.num_weights} \n"
                + f"Total outputs: {self.num_outputs} \n"
            )
    ...
    def gen_new_task_emb(self) -> None:
        # Bug fix: Initializing with torch.Tensor can produce nans, using torch.randn instead
        # which samples from a normal distribution with mean=0 and std=1
        # See https://discuss.pytorch.org/t/nn-parameter-contains-nan-when-initializing/44559/2
        chunked_task_emb = nn.Parameter(
            data=torch.randn((self.num_chunks, self.te_dim)), requires_grad=True
        )
        assert not (
            torch.isnan(chunked_task_emb).any()
        ), f"nans in TE {torch.isnan(chunked_task_emb).any()}"

        self.task_embs.append(chunked_task_emb)
    ...
    def forward(self, task_id, dTheta=None, squeeze=True):
        ...
        # The chunked hypernetwork has only 1 output head
        outputs = []
        j = 0
        for i in range(len(self.hidden_dims), len(self.theta_shapes), 2):
            W = F.linear(h, weights[i], bias=weights[i + 1])
...
class TargetNetwork(nn.Module):
    def __init__(
        ...
        linear_shapes = TargetNetwork.weight_shapes(
            n_in=n_in, n_out=n_out, hidden_layers=hidden_layers, use_bias=use_bias
        )
        ...
        for i, dims in enumerate(linear_shapes):
            self.weights.append(nn.Parameter(torch.Tensor(*dims), requires_grad=True))
            if len(dims) == 1:
        ...
        if init_weights is not None:
            ...
            for i in range(len(init_weights)):
                assert np.all(np.equal(list(init_weights[i].shape), linear_shapes[i]))
        ...
        for sample in dataloader:
            ...
            if calc_reg:
                ...
                loss_reg = calc_fix_target_reg(
                    hnet, task_id, targets=targets, dTheta=dTheta
                )

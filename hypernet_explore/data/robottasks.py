...
class RobotTasksPositionOrientation():
    ...
    def __init__(self, 
        ...
        # Project thscalee quaternion trajectories to the tangent plane
        ...
        if self.norm:
            ...
            self.position = torch.from_numpy(self.position)
        ...
        # Using the name pos so as not to break existing code
        # In reality pos contains a concatenation of position and orientation (mapped to the tangent plane)
        self.pos = torch.cat((self.position, self.rotation), -1)
        self.t = torch.from_numpy(np.array(self.t)).to(device)

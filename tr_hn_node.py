...
from hypernet_explore.train.utils import check_cuda, set_seed, get_sequence
from hypernet_explore.model.hypernetwork import HyperNetwork, TargetNetwork, str_to_ints, str_to_act, \
...
def train_task(args, task_id, hnet, tnet, node, device, writer=None):
    ...
    for training_iters in trange(args.num_iter):
        ...
        if loss.item() <= best_loss:
            ...
            #best_tnet = deepcopy(tnet.detach) # Not needed
            #best_node = deepcopy(node)  # Not needed
            best_loss = loss.item()
            best_iter = int(training_iters)        

    return best_hnet, tnet, node, best_loss, best_iter
...
def train_all(args):
    ...
    for task_id in range(num_tasks):
        ...
        hnet, tnet, node, best_loss, best_iter = train_task(args, task_id, hnet, tnet, node, device, writer=None)
...
def eval_during_train(args, save_dir, train_task_id, eval_results=None, writer=None):
    ...
    logging.info(f'#### final Evaluation started for task_id: {train_task_id} (task {train_task_id+1} out of {num_tasks}) ###')
    ...
    for eval_task_id in range(train_task_id+1):
        logging.info(f'final Loaded network trained on task {train_task_id}, evaluating on task {eval_task_id}')
        eval_traj_metrics, eval_traj_metric_errors = eval_task(args, eval_task_id, hnet, tnet, node, device, writer)
        
        logging.info(f'final Evaluated trajectory metrics: {eval_traj_metrics}')
...
def eval_all(args, save_dir):
    ...
    for task_id in range(num_tasks):
        ...
        for eval_task_id in range(task_id+1):
            ...
            eval_traj_metrics, eval_traj_metric_errors = eval_task(args, eval_task_id, hnet, tnet, node, device)

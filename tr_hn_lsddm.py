...
from hypernet_explore.logging.utils import custom_logging_setup, read_dict, write_dict, Dictobject, NumpyArrayEncoder
...
# TensorboardX warning
warnings.filterwarnings("ignore", message="NaN or Inf found in input tensor.")
...
def parse_args(return_parser=False):
    ...
    parser.add_argument('--tb_log_dir', type=str, default='runs/', help='Tensorboard logs')
...
def train_task(args, task_id, hnet, tnet, node, device, param_names, pbar=trange, writer=None):
...
def train_all(args):
    ...
    for task_id in range(num_tasks):
        ...
        hnet, duration, best_loss, best_iter = train_task(args, task_id, hnet, target_network, node, device, param_names, pbar=trange, writer=None)
